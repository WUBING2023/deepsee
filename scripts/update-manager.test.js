...
describe("DeepSee update manager", () => {
  ...
  it("turns invalid remote metadata into a retryable error", async () => {
    ...
    const status = await checkDeepSeeUpdate(root, packageRoot, {
      fetchImpl: updateFetch("0.6.0-alpha.8", { name: "@attacker/lookalike" }),
    });
  ...
  it("starts one detached automatic installer only after a verified check", async () => {
    const root = fixture();
    await checkDeepSeeUpdate(root, packageRoot, { fetchImpl: updateFetch("0.6.0-alpha.8") });
    ...
    expect(status).toMatchObject({ status: "updating", latestVersion: "0.6.0-alpha.8" });
  ...
  it("requires a manual install when a future update protocol is not supported", async () => {
    const root = fixture();
    const status = await checkDeepSeeUpdate(root, packageRoot, {
      fetchImpl: updateFetch("0.6.0-alpha.8", {
        deepsee: {
    ...
    expect(status).toMatchObject({ status: "manual-required", latestVersion: "0.6.0-alpha.8" });
  ...
  it("atomically replaces update state and leaves no temporary file", () => {
    ...
    writeDeepSeeUpdateState(root, { status: "available", latestVersion: "0.6.0-alpha.8" });
