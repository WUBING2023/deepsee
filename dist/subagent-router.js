const ROUTE_ID_PREFIXES = ["api:", "harness:", "cli:", "ocr:"];
function looksLikeRouteId(value) {
    return ROUTE_ID_PREFIXES.some((prefix) => value.startsWith(prefix));
}
function assertRunnable(route) {
    if (!route.enabled) {
    ...
    if (route.source === "cli" || route.source === "ocr") {
        throw new Error(`DeepSee route "${route.id}" is discovered but does not yet have a verified Harness child-agent adapter.`);
    }
}
/**
 ...
 */
export function resolveDeepSeeAgentOptions(registry, requested) {
