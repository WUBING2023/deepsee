...
export function cliRuntimeProviderId(routeId) {
    const slug = routeId
        ...
        .replace(/^-+|-+$/g, "");
    if (!slug)
        throw new Error(`DeepSee CLI route "${routeId}" has no usable provider id.`);
...
function routeModels(route) {
    ...
    if (selected && (models.length === 0 || models.includes(selected)))
        return [selected];
    if (models.length > 0)
        return [models[0]];
    return [route.model];
...
export function cliBasePrompt(options) {
...
export function cliTaskPrompt(options, route) {
    const prompt = [{ type: "text", text: cliBasePrompt(options) }];
...
export class CliRuntimeAdapter extends LlmAdapter {
    ctx;
    getRegistry;
    constructor(ctx, getRegistry) {
        super();
        this.ctx = ctx;
        this.getRegistry = getRegistry;
    }
    routes(provider) {
        const routes = this.getRegistry().routes.filter((candidate) => (candidate.source === "cli"
            && cliRuntimeProviderId(candidate.id) === provider
            && candidate.enabled
            && candidate.status === "ready"));
        if (routes.length === 0)
            throw new Error(`DeepSee CLI provider "${provider}" is not in the model registry.`);
    ...
    async *stream(options) {
        ...
        try {
            ...
            if (!text)
                throw new Error(`${route.id} base runtime returned no assistant text.`);
...
export function installCliRuntimeAdapters(ctx, getRegistry) {
    const providers = getRegistry().routes
        .filter((route) => (route.source === "cli"
        && route.enabled
        && route.status === "ready"
