...
安装前还会检查更新协议兼容性。未来版本如果改变包结构，可以声明更高的更新协议或最低更新器版本；旧版 DeepSee 会保持现有插件可用并显示 **需手动升级**，不会冒险执行不兼容迁移。更新状态采用原子替换；瞬时检查失败会在 15 分钟后自动重试，而不是被六小时缓存长期压住。Harness peer 依赖遵循官方当前使用的兼容 SemVer 范围，允许同一 `0.1.x` API 系列内的后续兼容发行版，同时拒绝未经验证的下一破坏性版本系列。
