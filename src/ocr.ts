...
function imageBlocks(content: readonly ContentBlock[]): Extract<ContentBlock, { type: "image" }>[] {
...
function runMinerU(executable: string, input: string, output: string, timeoutMs: number, signal?: AbortSignal): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    child.once("exit", (code) => {
      ...
      if (signal?.aborted) return reject(signal.reason || new Error("MinerU OCR 已取消。"));
      if (code === 0) resolve();
      else reject(new Error(stderr.trim() || `MinerU 退出码 ${String(code)}`));
    });
  });
}
...
function runPythonOCR(
  ...
  return new Promise((resolve, reject) => {
    ...
    child.once("exit", (code) => {
      clearTimeout(timer);
      signal?.removeEventListener("abort", abort);
      if (signal?.aborted) return reject(signal.reason || new Error(`${tool} OCR 已取消。`));
      if (code !== 0) return reject(new Error(stderr.trim() || `${tool} 退出码 ${String(code)}`));
      const marker = "__DEEPSEE_OCR__";
      const markerIndex = stdout.lastIndexOf(marker);
      if (markerIndex === -1) return reject(new Error(`${tool} 未返回 DeepSee OCR 协议结果。`));
      const line = stdout.slice(markerIndex + marker.length).split(/\r?\n/, 1)[0];
      try {
        const value = JSON.parse(line) as { texts?: unknown };
        const texts = Array.isArray(value.texts)
          ? value.texts.filter((item): item is string => typeof item === "string" && item.trim().length > 0)
          : [];
        resolve(texts.join("\n"));
      } catch {
...
function collectTextFiles(root: string): string[] {
  ...
  for (const entry of readdirSync(root, { withFileTypes: true })) {
    ...
    else if ([".md", ".txt"].includes(extname(entry.name).toLowerCase())) files.push(path);
  }
  return files;
}

export async function describeImagesWithMinerU(
  ctx: Pick<Context, "attachments">,
  message: UserMessage,
  config: MinerUConfig,
  signal?: AbortSignal,
): Promise<string> {
  if (!config.executable) return "[DeepSee OCR unavailable: MinerU 尚未安装]";
  const blocks = imageBlocks(message.content);
  if (blocks.length === 0) return "[DeepSee OCR: 没有图片输入]";
...
export async function describeImagesWithLocalOCR(
  ctx: Pick<Context, "attachments">,
  message: UserMessage,
  config: LocalOCRConfig,
  signal?: AbortSignal,
): Promise<string> {
  if (config.tool === "mineru") return describeImagesWithMinerU(ctx, message, config, signal);
  if (!config.executable) return `[DeepSee OCR unavailable: ${config.tool} 尚未安装]`;
  const blocks = imageBlocks(message.content);
  if (blocks.length === 0) return "[DeepSee OCR: 没有图片输入]";
