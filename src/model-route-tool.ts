...
function readyRoute(registry: ModelRegistryFile, routeId: string): ModelRoute {
  const route = registry.routes.find((candidate) => candidate.id === routeId);
  if (!route) throw new Error(`DeepSee route "${routeId}" was not found.`);
...
export async function runModelRoute(
  ...
  input: { route: string; prompt: string; model?: string },
  parent: Agent,
  signal: AbortSignal,
  inheritedGlobalMemory = "",
): Promise<{ route: string; model: string; text: string }> {
  const route = readyRoute(registry, input.route.trim());
  const requestedModel = input.model?.trim();
  let provider = DEEPSEE_SUBAGENT_PROVIDER;
  let agentOptions: { model: string } = { model: route.id };
  let selectedModel = route.runtimeModel || route.model;
  if (route.source === "cli") {
    if (!route.runtimeProvider) throw new Error(`${route.id} has no native Harness provider.`);
  ...
  try {
    ...
    if (!text) throw new Error(`${route.id} returned no assistant text.`);
...
export function installModelRouteTool(
  ctx: Context,
  ...
  ctx.tools.register(defineTool({
    ...
    async execute(args, exec) {
      if (!exec.agent) throw new Error("opends_run_model requires a live Harness agent session.");
